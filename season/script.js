...
// Add to cart function - now uses product ID properly
function addToCart(productId, selectedSize) {
  ...
  if (existingItem) {
    ...
    showAddToCartFeedback('Quantity updated in cart!');
  } else {
    cart.push({
      id: product.id,
      name: product.name,
    ...
    showAddToCartFeedback('Item added to cart!');
  }
...
// Improved feedback function with custom message
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (productGrid) {
    ...
    function renderProducts(season = "All", priceRange = "All", category = "All") {
      ...
      filteredProducts.forEach((product) => { // Removed index parameter
          ...
          <div class="product-info">
            ...
            <div class="product-actions">
              <button class="add-cart" data-id="${product.id}">Add to Cart</button>
               <button onclick="window.location.href='login.html'" class="buy-now">Buy Now</button>
            </div>
          </div>
        `;
      ...
      document.querySelectorAll('.buy-now').forEach(button => {
        button.addEventListener('click', function() {
          ...
          const productId = parseInt(productCard.querySelector('.add-cart').getAttribute('data-id'));
